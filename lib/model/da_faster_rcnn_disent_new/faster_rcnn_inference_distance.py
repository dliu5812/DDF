...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, src_ins_feat_list, tgt_ins_feat_list):
        ...
        heat_di = self.get_feat_map_vis(base_feat, size = (width, height))
        heat_ds = self.get_feat_map_vis(ds_feat_src, size= (width, height))
        ...
        tmp1, cls_pred = torch.max(cls_prob, dim=1)
        fg_idx = torch.nonzero(cls_pred).squeeze(1).detach()
        src_ins_feat_list = self.append_feat_to_list(pooled_feat, src_ins_feat_list, cls_pred, fg_idx )


        #
        #
        # print('src obj class, ', cls_pred)
        # print('fg idx, ', fg_idx)


        # print('cls score, ', cls_prob.size())
        # print('ins feat, ', pooled_feat.size())
        #
        # print('num of fg idx, ', fg_idx, fg_ins_feat_src.size())
        ...
        tmp1, cls_pred_tgt = torch.max(cls_prob_tgt, dim=1)
        fg_idx_tgt = torch.nonzero(cls_pred_tgt).squeeze(1).detach()
        tgt_ins_feat_list = self.append_feat_to_list(tgt_pooled_feat, tgt_ins_feat_list, cls_pred_tgt, fg_idx_tgt)


        """ =================== collect features =========================="""

        src_global_feat = F.avg_pool2d(base_feat, (base_feat.size(2), base_feat.size(3))).view(base_feat.size(1))
        tgt_global_feat = F.avg_pool2d(tgt_base_feat, (tgt_base_feat.size(2), tgt_base_feat.size(3))).view(tgt_base_feat.size(1))




        # print('base feat, ', base_feat.size(), src_global_feat.size())
        # print('ins feat, ',fg_ins_feat_src.size(), fg_ins_feat_tgt.size())

        #
        src_global_feat = src_global_feat.data.cpu().numpy()
        tgt_global_feat = tgt_global_feat.data.cpu().numpy()

        # src_ins_feat = torch.mean(fg_ins_feat_src, dim = 0).view(fg_ins_feat_src.size(1))
        # tgt_ins_feat = torch.mean(fg_ins_feat_tgt, dim=0).view(fg_ins_feat_tgt.size(1))

        # src_ins_feat = src_ins_feat.data.cpu().numpy()
        # tgt_ins_feat =  tgt_ins_feat.data.cpu().numpy()
        #
        # src_ins_feat = None
        # tgt_ins_feat = None

        # print('src box, ', rois.data[:, :, 1:5])
        # print('tgt box, ', tgt_rois.data[:, :, 1:5])

        return tgt_rois,  cls_prob_tgt, bbox_pred_tgt, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, \
                src_global_feat, tgt_global_feat, src_ins_feat_list, tgt_ins_feat_list
